	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.yanxueLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('yanxueLiuyan', this.yanxueLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.yanxueLiuyanPhoto ? this.detail.yanxueLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yanxueLiuyanYuyue', {
									yonghuId: _this.user.id,
									yanxueLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/yanxueLiuyanYuyue/list');
							}
						}
					});
				},
