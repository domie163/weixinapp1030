		...
		<view class="">
			<view class="price ">
				...
				<view class="pricejiner"
					  :style="detail.jingdianNewMoney>1000? 'width: 450rpx;':detail.jingdianNewMoney>10000? 'width: 550rpx;':'width: 400rpx;'">
					<text style='font-size:20rpx'>¥</text>
					<text style="font-weight:bold;font-size:50rpx">{{detail.jingdianNewMoney}}</text>
						<text style="font-size: 20rpx;color: #fff;margin-left: 10rpx;">
							<text v-if="detail.jingdianOldMoney!=null">原价￥
								<text style="text-decoration:line-through">{{detail.jingdianOldMoney}}</text>
							</text>
						</text>
				</view>
			</view>
			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.jingdianName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.jingdianContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 评价 -->
				...
				<view class="bottom-content cu-bar bg-white tabbar border shop">
					<view style="width: 100%;margin: 0 auto;display: flex;align-items: center;justify-content: space-around;">
						...
						<view @tap="onSubmit" style="display: flex; width: 80%;color: #FFFFFF;">
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.jingdianId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getjingdianCollection()
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('jingdian', this.jingdianId);
                    this.detail = res.data
					this.detail.jingdianContent = this.detail.jingdianContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.jingdianPhoto ? this.detail.jingdianPhoto.split(",") : [];
				},
				...
				async getjingdianCollection() {
					...
					let res = await this.$api.page(`jingdianCollection`, params);
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`jingdianCollection`, params);
					if (res.data.list.length == 1) {
						let jingdianCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('jingdianCollection', JSON.stringify([jingdianCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getjingdianCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jingdianCollection', {
									yonghuId: _this.user.id,
									jingdianId: _this.detail.id,
									jingdianCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('jingdianCommentback', {
						page: mescroll.num,
						limit: mescroll.size,
						jingdianId: this.jingdianId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('jingdian', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jingdianYuyue', {
									yonghuId: _this.user.id,
									jingdianId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/jingdianYuyue/list');
							}
						}
					});
				},
