				...
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.jingdianCollectionValue}}
							</view>
						</view>
					</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.jingdianCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('jingdianCollection', this.jingdianCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.jingdianCollectionPhoto ? this.detail.jingdianCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jingdianCollectionYuyue', {
									yonghuId: _this.user.id,
									jingdianCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/jingdianCollectionYuyue/list');
							}
						}
					});
				},
