	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.jingdianOrderId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('jingdianOrder', this.jingdianOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.jingdianOrderPhoto ? this.detail.jingdianOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jingdianOrderYuyue', {
									yonghuId: _this.user.id,
									jingdianOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/jingdianOrderYuyue/list');
							}
						}
					});
				},
