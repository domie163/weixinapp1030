    ...
    <view class="content">
        <form class="app-update-pv">
                ...
                <view :style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"100rpx"}'
                    ...
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">类型</view>
                    <picker @change="yanxueCollectionTypesChange" :value="yanxueCollectionTypesIndex" :range="yanxueCollectionTypesOptions" range-key="indexName">
                        <view
                                ...
                                class="uni-input">{{ruleForm.yanxueCollectionTypes?ruleForm.yanxueCollectionValue:"请选择类型"}}</view>
                    </picker>
                </view>
...
<script>
    ...
    export default {
		...
		/*下拉框*/
			let yanxueCollectionTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'yanxue_collection_types',
            }
			let yanxueCollectionTypes = await this.$api.page(`dictionary`, yanxueCollectionTypesParams);
			this.yanxueCollectionTypesOptions = yanxueCollectionTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`yanxueCollection`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.yanxueCollectionId){
                this.ruleForm.yanxueCollectionId = options.yanxueCollectionId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
                if (this.ruleForm.id) {
                    await this.$api.update(`yanxueCollection`, this.ruleForm);
                } else {
                    await this.$api.save(`yanxueCollection`, this.ruleForm);
                }
