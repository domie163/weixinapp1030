	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('yanxueYuyue', this.yanxueYuyueId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.yanxueYuyuePhoto ? this.detail.yanxueYuyuePhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yanxueYuyueYuyue', {
									yonghuId: _this.user.id,
									yanxueYuyueId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/yanxueYuyueYuyue/list');
							}
						}
					});
				},
