	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.yanxueYuyueText" type="text" placeholder="报名理由"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('yanxueYuyue','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('yanxueYuyue','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.yanxueYuyueName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('yanxueYuyue','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <view v-show="item.yanxueName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">研学标题</text>
                    <text style="width: 70%">{{item.yanxueName}}</text>
                </view>
                <view v-show="item.yanxueTypes" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">研学类型</text>
                    <text style="width: 70%">{{item.yanxueValue}}</text>
                </view>
                ...
                <!-- 当前表 -->
                 <view v-show="item.yanxueYuyueUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">报名编号</text>
                     <text style="width: 70%">
                         <text v-if="item.yanxueYuyueUuidNumber != null">{{item.yanxueYuyueUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                <view v-show="item.yanxueYuyueText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">报名理由</text>
                    <view style="width: 70%" v-html="item.yanxueYuyueText"></view>
                </view>
                 <view v-show="item.yanxueYuyueYesnoValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">报名状态</text>
                     <text style="width: 70%">{{item.yanxueYuyueYesnoValue}}</text>
                 </view>
                <view v-show="item.yanxueYuyueYesnoText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">审核回复</text>
                    <view style="width: 70%" v-html="item.yanxueYuyueYesnoText"></view>
                </view>
                <view v-show="item.yanxueYuyueShenheTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">审核时间</text>
                    <text style="width: 70%">
                        <text v-if="item.yanxueYuyueShenheTime != null">{{item.yanxueYuyueShenheTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('yanxueYuyue','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('yanxueYuyue','审核') && item.yanxueYuyueYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`yanxueYuyue`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yanxueYuyue', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.yanxueYuyueShenheTime=shijian;
                            await _this.$api.update(`yanxueYuyue`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.yanxueYuyueUuidNumber) {
                    searchForm['yanxueYuyueUuidNumber'] = this.searchForm.yanxueYuyueUuidNumber
                }
				if (this.searchForm.yanxueYuyueText) {
                    searchForm['yanxueYuyueText'] = this.searchForm.yanxueYuyueText
                }

                let res = await this.$api.page(`yanxueYuyue`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
