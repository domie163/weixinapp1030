	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.jingdianCommentbackId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('jingdianCommentback', this.jingdianCommentbackId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.jingdianCommentbackPhoto ? this.detail.jingdianCommentbackPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jingdianCommentbackYuyue', {
									yonghuId: _this.user.id,
									jingdianCommentbackId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/jingdianCommentbackYuyue/list');
							}
						}
					});
				},
