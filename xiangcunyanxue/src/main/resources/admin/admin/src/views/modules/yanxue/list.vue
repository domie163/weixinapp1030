        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('yanxue','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiangcunyanxue/upload/yanxueMuBan.xls"
                        >批量导入研学数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yanxue','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="yanxueUploadSuccess"
                                :on-error="yanxueUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('yanxue','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "yanxue.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="yanxueKucunNumber"
                                      header-align="center"
                                      label="剩余可参与人数">
                        <template slot-scope="scope">
                            {{scope.row.yanxueKucunNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('yanxue','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('yanxue','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('yanxue','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('yanxue','修改')" type="primary" icon="el-icon-tickets" size="mini" @click="shangxia(scope.row.id,scope.row.shangxiaTypes)">{{scope.row.shangxiaTypes == 1?'下架':'上架'}}</el-button>
                            <!--<el-button v-if="isAuth('yanxue','修改')" type="primary" icon="el-icon-add-location" size="mini" @click="plusYanxueKucunNumber(scope.row.id,scope.row.yanxueKucunNumber)">增加剩余可参与人数</el-button>-->
                            <!--<el-button v-if="isAuth('yanxue','修改')" type="primary" icon="el-icon-delete-location" size="mini" @click="reduceYanxueKucunNumber(scope.row.id,scope.row.yanxueKucunNumber)">减少剩余可参与人数</el-button>-->
...
<!-- 库存 -->
		<el-dialog
                :title="yanxueKucunNumberTitle"
                :visible.sync="yanxueKucunNumberVisible"
                width="30%">
            <span>操作数量</span><el-input-number v-model="kucunNumber" :min="1" :max="100" label="描述文字"></el-input-number>
            <span slot="footer" class="dialog-footer">
			<el-button @click="yanxueKucunNumberVisible = false">取 消</el-button>
			<el-button type="primary" @click="addYanxueKucunNumber()">确 定</el-button>
		  </span>
        </el-dialog>
...
<script>
    ...
    //当前表下拉框搜索条件
        ...
        //库存
            ...
            yanxueKucunNumber: 0,
            //标题
            yanxueKucunNumberTitle: null,
            //模态框状态
            yanxueKucunNumberVisible: false,
            };
        },
        created() {
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'yanxue',//当前表表名,
                        sumColum : 'yanxue_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'yanxue_name',//分组字符串字段
                        // types : 'yanxue_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['yanxueDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            yanxueUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "yanxue/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            yanxueUploadError(data){
                this.$message.error('上传失败');
            },
            //增加剩余可参与人数
			plusYanxueKucunNumber(thisId,yanxueKucunNumber){
			//清空数据
				//剩余可参与人数数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				//将模态框打开
				this.yanxueKucunNumberVisible = true;
				//标题
				this.yanxueKucunNumberTitle = '增加剩余可参与人数'
			},
			//减少剩余可参与人数
			reduceYanxueKucunNumber(thisId,yanxueKucunNumber){
			//清空数据
				//剩余可参与人数数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				this.yanxueKucunNumberVisible = true;
				//标题
				this.yanxueKucunNumberTitle = '减少剩余可参与人数'
			},
			//提交用户操作剩余可参与人数数量
			addYanxueKucunNumber(){
				var type = null
				var maxNumber = 0
				if(this.kucunNumberTypes == "+"){
					type = '增加'
					maxNumber = this.yanxueKucunNumber + this.kucunNumber
				}else{
					type = '减少'
					if(this.yanxueKucunNumber < this.kucunNumber || this.kucunNumber <= 0){
						this.$message.error('剩余可参与人数不足不能减少');
						return false;
					}else{
						maxNumber = this.yanxueKucunNumber - this.kucunNumber
					}
				...
				this.yanxueKucunNumberVisible = false;
			},	
			//上下架
